...
- `dojo/<bucket>/skill/personas/<slug>/persona.md` + `topics/` — **the actual expert files** that ship.
- `dojo/buckets.json` — **single source of truth** for bucket structure (label, description, ordering).
- `tools/` — the build & sync scripts (sync-public, generate-domain-skills, gap-check, etc.).
...
- `dojo/<bucket>/skill/` — mirrored from private, no raw sources, no extractions.
- `skills/dojo-<slug>/` — pre-built per-persona skill packages, regenerated by `build-granular-skills.ts`.
...
|---------------------------------|-------------------------------------------------------|---------------------------------|
| Persona content / topics / voice| `dojo/<bucket>/skill/personas/<slug>/` in private     | `make sync`                     |
| Bucket names / labels / order   | `dojo/buckets.json` in private                        | `make sync`                     |
| Topic taxonomy (TOPIC union)    | `dojo-builder/scripts/build-manifest.ts` (TOPICS arr) | `npm run build:manifest`        |
| Per-persona TOPIC_MAP           | `dojo-builder/scripts/build-manifest.ts` (TOPIC_MAP)  | `npm run build:manifest`        |
...
public/dojo-builder/scripts/build-granular-skills
        │
        ├── writes public/skills/dojo-<slug>/
